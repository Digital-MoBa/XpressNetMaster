...
RS485SoftwareSerial.cpp - Implementation of a sofware 9bit RS485 based on 
...
RS485SoftwareSerial *ObjList[MAX_PIN + 1];
...
void RS485SoftwareSerial::setup(int receivePin, int transmitPin, bool inverse_logic, unsigned int buffSize)
{
  ...
  if (isValidGPIOpin(receivePin))
  {
    m_rxPin = receivePin;
    m_buffSize = buffSize;
    m_buffer = (uint16_t *)malloc(m_buffSize*2);
    if (m_buffer != NULL)
    {
      m_rxValid = true;
      m_inPos = m_outPos = 0;
...
long RS485SoftwareSerial::baudRate()
{
...
void RS485SoftwareSerial::enableRx(bool on)
{
  if (m_rxValid)
  {
    if (on)
...
int RS485SoftwareSerial::available()
{
  if (!m_rxValid)
    return 0;
...
size_t RS485SoftwareSerial::write(uint16_t b)
{
  if (!m_txValid)
    return 0;
...
void RS485SoftwareSerial::flush()
{
  m_inPos = m_outPos = 0;
}

bool RS485SoftwareSerial::overflow()
{
  bool res = m_overflow;
  m_overflow = false;
  return res;
...
void ICACHE_RAM_ATTR RS485SoftwareSerial::rxRead()
{
